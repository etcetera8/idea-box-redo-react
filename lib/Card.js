...
export default class Card extends Component {
  ...
  downvoter() {
    ...
    if (this.state.qual === "genius") {
      this.setState({qual: "good"});
      retrieved.qual = "good";
      localStorage.setItem(this.state.key, JSON.stringify(retrieved));
    }
    if (this.state.qual === "good") {
      this.setState({qual: "swill"});
      retrieved.qual = "swill";
      localStorage.setItem(this.state.key, JSON.stringify(retrieved));
    }
  }
    ...
    return (
      <div className='Card'>
        <span 
          ...
          onClick={() => this.props.delete(this.props.title)}>x
        </span>
        <h3>{this.props.title}</h3>
        <p>{this.props.body}</p>
        <div className='vote'>
          <span id='upvote' onClick={() => this.upvoter()}>↑</span>
          <span id='downvote' onClick={() => this.downvoter()}>↓</span>
